...
Due to file size constraints, the curated image datasets, checkerboard images, final output **Masked Images**, and trained model weights (`.pth` files) are hosted externally. 
...
## 🚀 How to Test
Since the model is evaluated locally rather than deployed online, you can test the pipeline by running the final inference script:

1. **Install Dependencies:** Run `pip install -r requirements.txt` in your terminal.
2. **Download Weights:** Download the `.pth` model weights and test images from the Google Drive link above.
3. **Configure Local Paths:** Open `inference/Final_Inference.py` in your code editor. Update the local file paths for both the target input picture and the downloaded model weights.
4. **Run Inference:** Execute the script. The final measurements and masked image outputs will be generated upon running with the correct file paths.

---

## 🏗️ Repository Structure & Documentation
To maintain a clean working directory, all detailed technical reports, limitations, and error analyses have been centralized in the `docs/` folder[cite: 1].
